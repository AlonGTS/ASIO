...
def _load_multicast_group():
    """Must match tracker-so.py's video_multicast_group — the Pi sends one
    stream to this group, any number of GCS/viewer instances can join it.
    Fallback here must match video_capture.DEFAULT_MULTICAST_GROUP."""
    cfg_path = Path(__file__).parent / "config.toml"
    if cfg_path.exists():
        try:
            import tomllib
            with open(cfg_path, "rb") as f:
                cfg = tomllib.load(f)
            return cfg["network"].get("video_multicast_group", "239.5.5.5")
        except Exception:
            pass
    return "239.5.5.5"
...
UDP_PORT = args.udp
MCAST_GROUP = _load_multicast_group()
...
print(f"[GCS] Pi={PI_IP}  video={MCAST_GROUP}:{UDP_PORT} (multicast)  cmd={CMD_PORT}")
...
# capture classes (_LiveCapture/_H264LiveCapture, imported from
# video_capture.py further down) aren't in scope yet at this point in the
...
def toggle_video_mode():
    ...
    cap.close()
    cap = _H264LiveCapture(UDP_PORT, MCAST_GROUP) if new_mode == "h264_udp" else _LiveCapture(UDP_PORT, MCAST_GROUP)
...
# ── Live capture (video_capture.py) ─────────────────────────────────────────
#
# LiveCapture (jpeg_udp) / H264LiveCapture (h264_udp) — extracted into a
# shared module so viewer.py can reuse the exact same receivers without
# pulling in this file's control UI. Aliased back to the old in-file names
# below so every existing call site here is unchanged.

from video_capture import LiveCapture as _LiveCapture, H264LiveCapture as _H264LiveCapture
...
def main():
    ...
    if args.file:
        ...
        cap = _H264LiveCapture(UDP_PORT, MCAST_GROUP) if video_mode == "h264_udp" else _LiveCapture(UDP_PORT, MCAST_GROUP)
